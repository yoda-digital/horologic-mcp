import { ZonedDateTime, LocalTime, ZoneId, DateTimeFormatter } from '@js-joda/core';
...
export class TimeService {
   ...
   */
  private getZoneId(timezone: string): ZoneId {
    try {
  ...
  convertTime(sourceTimezone: string, timeStr: string, targetTimezone: string): TimeConversionResult {
    ...
    const localTime = LocalTime.of(hours, minutes);
    
    // Get the current date in the source timezone
    const sourceNow = ZonedDateTime.now(sourceZoneId);
    ...
    return {
      source: {
        ...
        datetime: sourceTime.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
        is_dst: this.isInDaylightSavings(sourceTime),
      },
      target: {
        timezone: targetTimezone,
        datetime: targetTime.format(DateTimeFormatter.ISO_OFFSET_DATE_TIME),
   ...
   */
  private isInDaylightSavings(dateTime: ZonedDateTime): boolean {
    try {
